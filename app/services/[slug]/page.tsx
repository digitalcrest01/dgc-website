  ...
  return (
    <>
        ...
        <div className="container-page relative">
          ...
          <div className="mt-6 grid gap-10 lg:grid-cols-[1.4fr_1fr]">
            <div>
              ...
              <h1 className="mt-5 font-display text-4xl font-semibold tracking-tight text-navy text-balance sm:text-5xl">
                {service.name}
              </h1>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_2fr]">
          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {service.outcomes.map((o) => (
              <li
                key={o}
                className="flex items-start gap-3 rounded-xl border border-navy/10 bg-cream-100/60 p-4"
              >
      ...
      <section className="container-page mt-20 lg:mt-24">
        ...
        <h2 className="mt-3 max-w-3xl font-display text-2xl font-semibold tracking-tight text-navy sm:text-3xl">
          The work, broken down.
        </h2>

        <div className="mt-8 grid gap-px overflow-hidden rounded-2xl border border-navy/10 bg-navy/10 sm:grid-cols-2">
          {service.capabilities.map((c, i) => (
            <div key={c.title} className="bg-cream-100 p-6">
              <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-navy/45">
                {String(i + 1).padStart(2, '0')}
              </p>
              <h3 className="mt-2 font-display text-base font-semibold tracking-tight text-navy">
                {c.title}
              </h3>
              <p className="mt-2 text-sm leading-relaxed text-navy/70">{c.detail}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="container-page mt-20 lg:mt-24">
        <div className="rounded-2xl border border-navy/10 bg-cream-100/60 p-6 lg:p-10">
          ...
          {others.map((s) => (
            <Link key={s.slug} href={`/services/${s.slug}`} className="card group flex items-start justify-between">
              <div>
                ...
                <p className="mt-1.5 font-display text-base font-semibold tracking-tight text-navy">
                  {s.shortName}
                </p>
              </div>
    ...
    <div className="relative">
      ...
      <div className="overflow-hidden rounded-2xl border border-navy/15 bg-navy text-cream-100 shadow-2xl shadow-navy/20">
        <div className="flex items-center justify-between border-b border-cream-100/10 px-4 py-2.5">
          ...
          <p className="font-mono text-[10px] text-cream-100/40">{service.index}</p>
        </div>
